import type { AppEvent, BranchState, BookItem, RecommendationItem } from './types';
  ...
  for (const event of events) {
    ...
    switch (event.type) {
      case 'RECOMMENDATIONS_GENERATED': {
        ...
        const newItems: BookItem[] = payload.items.map((item: RecommendationItem) => ({
          title: item.title,
          author: item.author,
          reason: item.reason,
          status: 'PENDING',
      ...
      case 'ITEM_STATUS_CHANGED': {
        ...
        // Find in inbox first
        const inboxIndex = state.inbox.findIndex(item => item.title === itemTitle);
        if (inboxIndex !== -1) {
          ...
          if (status === 'ACCEPTED' || status === 'DEFERRED') {
            ...
            const libraryItem = state.library.find(item => item.title === itemTitle);
            if (libraryItem) {
                if (status === 'REJECTED') {
                    state.library = state.library.filter(i => i.title !== itemTitle);
                } else {
