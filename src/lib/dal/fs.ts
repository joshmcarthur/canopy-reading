...
const DATA_DIR = process.env.CANOPY_DATA_DIR || path.join(process.cwd(), 'data');
...
export async function getNextEventSequence(slug: string): Promise<number> {
  ...
  const seqs = eventFiles.map(f => parseInt(f.split('-')[0], 10));
  return Math.max(0, ...seqs) + 1;
}

export async function appendEvent(slug: string, event: AppEvent, bodyContent: string = '') {
