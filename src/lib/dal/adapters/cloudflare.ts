...
export class CloudflareAdapter implements StorageAdapter {
  ...
  async writeBranchMeta(slug: string, meta: Branch): Promise<void> {
    ...
    await stub.fetch(new Request(`https://branch.local/meta`, {
      method: 'POST',
    ...
    await registryStub.fetch(new Request(`https://registry.local/register`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(meta),
    }));
  ...
  async listBranches(): Promise<Branch[]> {
    const registryStub = await this.getBranchRegistryDOStub();
    const response = await registryStub.fetch(new Request(`https://registry.local/list`));
  ...
  async appendEvent(slug: string, event: AppEvent): Promise<void> {
    const stub = await this.getBranchDOStub(slug);
    await stub.fetch(new Request(`https://branch.local/events`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async readEvents(slug: string): Promise<AppEvent[]> {
    const stub = await this.getBranchDOStub(slug);
    const response = await stub.fetch(new Request(`https://branch.local/events`));
