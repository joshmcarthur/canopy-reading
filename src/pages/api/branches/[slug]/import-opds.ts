...
import { getBranch, addEvent } from '../../../../lib/dal';
import { opdsCatalogToRecommendationItems, type OPDSCatalog } from '../../../../lib/opds';
...
export const POST: APIRoute = async ({ params, request }) => {
  ...
  try {
    ...
    // Validate status
    const validStatuses: ItemStatus[] = ['PENDING', 'ACCEPTED', 'DEFERRED', 'REJECTED', 'ALREADY_READ'];
    ...
    if (!catalog.publications || !Array.isArray(catalog.publications)) {
      return new Response(JSON.stringify({ error: 'Invalid OPDS catalog structure. Missing publications array.' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' },
      });
    }
    ...
    if (items.length === 0) {
      return new Response(JSON.stringify({ error: 'No valid books found in OPDS catalog' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' },
      });
    }
