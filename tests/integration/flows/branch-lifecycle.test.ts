...
describe('Branch Lifecycle E2E Flow', () => {
  ...
  it('should complete full branch lifecycle: create → generate → accept → mark read', async () => {
    ...
    const branch = await createTestBranch('E2E Test Branch', 'Testing the full lifecycle');

    // Step 2: Generate recommendations
    const { POST } = await import('../../../src/pages/api/branches/[slug]/generate');
    const generateRequest = new Request(`http://localhost/api/branches/${branch.slug}/generate`, {
      method: 'POST',
    });
    ...
    expect(state.inbox.some(item => item.title === 'The Test Book')).toBe(true);

    // Step 3: Accept a book
    const { POST: statusPOST } = await import('../../../src/pages/api/branches/[slug]/status');
    const acceptFormData = new FormData();
    acceptFormData.append('itemTitle', 'The Test Book');
    acceptFormData.append('status', 'ACCEPTED');
    const acceptRequest = new Request(`http://localhost/api/branches/${branch.slug}/status`, {
      method: 'POST',
      body: acceptFormData,
    });
    ...
    expect(state.inbox.find(item => item.title === 'The Test Book')).toBeUndefined();
    const acceptedBook = state.library.find(item => item.title === 'The Test Book');
    ...
    const readRequest = new Request(`http://localhost/api/branches/${branch.slug}/status`, {
      method: 'POST',
      body: readFormData,
    });
    ...
    const bookInstances = state.library.filter(item => item.title === 'The Test Book');
    ...
    const readBook = bookInstances.find(item => item.status === 'ALREADY_READ');
    ...
    expect(reflectionEvents[reflectionEvents.length - 1].payload.content).toBe('This was an excellent book!');
    ...
    const finalStatusEvents = events.filter(e => 
      e.type === 'ITEM_STATUS_CHANGED' && 
      e.payload.itemTitle === 'The Test Book'
    );
    const alreadyReadEvent = finalStatusEvents.find(e => e.payload.status === 'ALREADY_READ');
    ...
    const opdsRequest = new Request(`http://localhost/api/branches/${branch.slug}/opds`);
  ...
  it('should handle multiple books in lifecycle', async () => {
    const branch = await createTestBranch('Multi Book Branch', 'Testing multiple books');

    // Generate recommendations
    const { POST: generatePOST } = await import('../../../src/pages/api/branches/[slug]/generate');
    await generatePOST({
      params: { slug: branch.slug },
      request: new Request(`http://localhost/api/branches/${branch.slug}/generate`, { method: 'POST' }),
      redirect: (url: string) => new Response(null, { status: 302, headers: { Location: url } }),
    } as any);
    ...
    await statusPOST({
      params: { slug: branch.slug },
      request: new Request(`http://localhost/api/branches/${branch.slug}/status`, {
        method: 'POST',
        body: acceptForm1,
      }),
      redirect: (url: string) => new Response(null, { status: 302, headers: { Location: url } }),
    ...
    expect(state.library.find(b => b.title === 'The Test Book')?.status).toBe('ACCEPTED');
    expect(state.library.find(b => b.title === 'Another Book')?.status).toBe('DEFERRED');
    ...
    const opds = await opdsResponse.json();
    const opdsTitles = opds.publications.map((p: any) => p.metadata.title);
