...
import type { OPDSCatalog } from '../../../src/lib/opds';
...
describe('OPDS API Integration', () => {
  ...
  it('should return OPDS catalog with correct structure', async () => {
    const recommendations = [
      createMockRecommendation('Book A', 'Author A', 'Why you should read Book A', {
        coverImageUrl: 'https://example.com/cover.jpg',
        isbn13: '9780123456789',
        openLibraryWorkKey: '/works/OL123W',
      }),
      createMockRecommendation('Book B', 'Author B', 'Why you should read Book B'),
    ];
    ...
    const branch = await createTestBranch('Test Branch OPDS', 'A test branch for OPDS', [
      generatedEvent,
      acceptedEvent1,
      acceptedEvent2,
    ]);
    ...
    const { GET } = await import('../../../src/pages/api/branches/[slug]/opds');
    const request = new Request(`http://localhost/api/branches/${branch.slug}/opds`);
  ...
  it('should include book metadata in publications', async () => {
    ...
    const acceptedEvent = createStatusChangedEvent('Book with Metadata', 'ACCEPTED');
    
    const branch = await createTestBranch('Test Branch Metadata', 'A test branch', [
      generatedEvent,
      acceptedEvent,
    ]);
  ...
  it('should handle books without metadata gracefully', async () => {
    const bookItem = createMockRecommendation('Simple Book', 'Author', 'Reason');
    const generatedEvent = createRecommendationsGeneratedEvent([bookItem]);
    const acceptedEvent = createStatusChangedEvent('Simple Book', 'ACCEPTED');
    
    const branch = await createTestBranch('Test Branch Simple', 'A test branch', [
      generatedEvent,
      acceptedEvent,
    ]);
  ...
  it('should return empty publications array when no to-read books', async () => {
    ...
    const branch = await createTestBranch('Test Branch Empty', 'A test branch', [
      generatedEvent,
      readEvent,
    ]);
  ...
  describe('OPDS Import API', () => {
     ...
     */
    function createOPDSCatalog(publications: OPDSCatalog['publications'] = []): OPDSCatalog {
      return {
        metadata: {
     ...
     */
    function createImportFormData(catalog: OPDSCatalog, status: string = 'ALREADY_READ'): FormData {
      const formData = new FormData();
      const blob = new Blob([JSON.stringify(catalog)], { type: 'application/json' });
      const file = new File([blob], 'catalog.json', { type: 'application/json' });
    ...
    it('should return 400 if slug is missing', async () => {
      const { POST } = await import('../../../src/pages/api/branches/[slug]/import-opds');
      ...
      const request = new Request('http://localhost/api/branches//import-opds', {
        method: 'POST',
        body: formData,
      });
    ...
    it('should return 404 if branch does not exist', async () => {
      ...
      const request = new Request('http://localhost/api/branches/nonexistent/import-opds', {
        method: 'POST',
        body: formData,
      });
    ...
    it('should return 400 if file is missing', async () => {
      ...
      const request = new Request(`http://localhost/api/branches/${branch.slug}/import-opds`, {
        method: 'POST',
        body: formData,
      });
    ...
    it('should return 400 if OPDS catalog structure is invalid', async () => {
      ...
      const formData = createImportFormData(invalidCatalog as any);
    ...
    it('should successfully import books with PENDING status', async () => {
      ...
      const catalog = createOPDSCatalog([
        {
          ...
          images: [{ href: 'https://example.com/cover1.jpg', type: 'image/jpeg' }],
          links: [
            {
      ...
      const statusChangeEvents = events.filter(e => e.type === 'ITEM_STATUS_CHANGED');
      ...
      expect(state.inbox[0].metadata?.coverImageUrl).toBe('https://example.com/cover1.jpg');
    ...
    it('should successfully import multiple books with different statuses', async () => {
      ...
      const request1 = new Request(`http://localhost/api/branches/${branch.slug}/import-opds`, {
        method: 'POST',
        body: formData1,
      });
      ...
      const request2 = new Request(`http://localhost/api/branches/${branch.slug}/import-opds`, {
        method: 'POST',
        body: formData2,
      });
      ...
      expect(state.library.find(b => b.title === 'Accepted Book')?.status).toBe('ACCEPTED');
      expect(state.library.find(b => b.title === 'Deferred Book')?.status).toBe('DEFERRED');
    });
      ...
      const catalog = createOPDSCatalog([
        ...
        },
        {
          metadata: {
            ...
            author: [{ name: 'Author' }],
          } as any, // Type assertion to allow testing invalid data (missing required title)
          links: [],
        },
      ]);
      const formData = createImportFormData(catalog, 'ACCEPTED');
    ...
    it('should extract metadata correctly from OPDS publications', async () => {
      ...
      expect(item.metadata?.coverImageUrl).toBe('https://example.com/cover.jpg');
      expect(item.metadata?.openLibraryWorkKey).toBe('/works/OL123W');
      expect(item.metadata?.description).toBe('A comprehensive book description');
    });
  });
});
