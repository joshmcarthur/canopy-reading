...
describe('State API Integration', () => {
  ...
  it('should return state for existing branch', async () => {
    ...
    const branch = await createTestBranch('State Test Branch', 'A test branch', [
      generatedEvent,
      acceptedEvent,
    ]);
  ...
  it('should return empty state for branch with no events', async () => {
    const branch = await createTestBranch('Empty State Branch', 'A branch with no events');
  ...
  it('should correctly project state with multiple status changes', async () => {
    ...
    const branch = await createTestBranch('State Multi Change Branch', 'A test branch', [
      generatedEvent,
      acceptedA,
      deferredB,
      readA,
  ...
  it('should handle rejected items correctly', async () => {
    ...
    const branch = await createTestBranch('State Rejected Branch', 'A test branch', [
      generatedEvent,
      acceptedEvent,
      rejectedEvent,
    ]);
    ...
    expect(state.inbox).toHaveLength(0);
    const rejectedInLibrary = state.library.find((b: any) => b.title === 'Rejected Book');
