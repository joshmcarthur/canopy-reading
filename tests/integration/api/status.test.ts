...
import { resetStorage, createTestBranch, createRecommendationsGeneratedEvent, createMockRecommendation, createStatusChangedEvent } from '../helpers';
...
describe('Status API Integration', () => {
  ...
  it('should return 400 if itemTitle or status is missing', async () => {
    ...
    const request1 = new Request(`http://localhost/api/branches/${branch.slug}/status`, {
      method: 'POST',
      body: formData1,
    });
    ...
    const request2 = new Request(`http://localhost/api/branches/${branch.slug}/status`, {
      method: 'POST',
      body: formData2,
    });
  ...
  it('should change item status from PENDING to ACCEPTED', async () => {
    ...
    const branch = await createTestBranch('Test Branch Status', 'A test branch', [generatedEvent]);
  ...
  it('should change item status to DEFERRED', async () => {
    ...
    const generatedEvent = createRecommendationsGeneratedEvent(recommendations);
    const branch = await createTestBranch('Test Branch Deferred', 'A test branch', [generatedEvent]);
  ...
  it('should change item status to REJECTED', async () => {
    ...
    const generatedEvent = createRecommendationsGeneratedEvent(recommendations);
    const branch = await createTestBranch('Test Branch Rejected', 'A test branch', [generatedEvent]);
  ...
  it('should add reflection when provided', async () => {
    ...
    const generatedEvent = createRecommendationsGeneratedEvent(recommendations);
    const acceptedEvent = createStatusChangedEvent('Test Book Reflection', 'ACCEPTED');
    const branch = await createTestBranch('Test Branch Reflection', 'A test branch', [
      generatedEvent,
      acceptedEvent,
    ]);
  ...
  it('should change status from DEFERRED to ACCEPTED', async () => {
    ...
    const generatedEvent = createRecommendationsGeneratedEvent(recommendations);
    const deferredEvent = createStatusChangedEvent('Test Book Status Change', 'DEFERRED');
    const branch = await createTestBranch('Test Branch Status Change', 'A test branch', [
      generatedEvent,
      deferredEvent,
    ]);
