...
import type { RecommendationsRequestedEvent } from "../src/domain/types";
...
describe("Data Access Layer", () => {
	...
	it("should create and retrieve a branch", async () => {
		const { createBranch, getBranch, getAllBranches } = await import(
			"../src/lib/dal/index"
		);
	...
	it("should add and retrieve events", async () => {
		const { createBranch, addEvent, getBranchEvents } = await import(
			"../src/lib/dal/index"
		);
		...
		const requestedEvent = events[1] as RecommendationsRequestedEvent;
		expect(requestedEvent.payload.userNote).toBe("Looking for dragons.");
	});
});
